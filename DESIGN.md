...
- **Hero (Landing):** grid `lg:grid-cols-[1.05fr_0.95fr]` — teks rata tengah di mobile, rata kiri di desktop; section `pt-14 sm:pt-20 pb-16 sm:pb-24`; headline via [TextAnimate](src/components/TextAnimate.tsx) (`blurInUp` per kata, `text-4xl sm:text-6xl lg:text-7xl`); kolase dua kolom foto nyata (`marquee-y` 36s + `marquee-y-slow` reverse 52s, `mask-fade-y`, `aria-hidden` — akses lewat galeri); CTA primer `size lg min-w-[180px] hover:-translate-y-0.5` (GPU, 200ms); link sekunder `.skiper-link py-1`; trust badges `font-mono text-xs`.
