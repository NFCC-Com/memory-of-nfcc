...
- **Navbar + menu (Skiper13):** bar `rounded-2xl bg-white/80 backdrop-blur-md shadow-[0_8px_30px_rgba(0,0,0,0.08)]`; menu terbuka = tiga blok terpisah bergap 12px — `[HEADER bar]` + `[NAVIGATION card]` + `[LANGUAGE card]`; kedua kartu `rounded-2xl border-[#EAEAEA] bg-white/95 backdrop-blur-md`, link `text-2xl sm:text-3xl font-extrabold uppercase tracking-tighter text-[#111111]`, item `hover:translate-x-2 hover:bg-[#F7F6F3]`, toggle bahasa segmented teks ID/ENG tanpa flag; area scroll kartu nav memakai `.no-scrollbar` (`src/index.css`). Lihat [skiper13.tsx](src/components/ui/skiper-ui/skiper13.tsx).
